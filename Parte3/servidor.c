...
void init_ipc() {
    ...
    // O semáforo em questão será usado com o padrão “Mutex”, pelo que será iniciado com o valor 1;
    int sem = semctl(sem_id, 0, SETVAL, 1);
    exit_on_error(sem, "S1) Semáforo com a Key definida não pode ser iniciado com o valor 1");
...
void init_database() {
    ...
    // • Lê o ficheiro FILE_CIDADAOS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->cidadaos e db->num_cidadaos. Se não o conseguir, dá erro e termina com exit status 1;
    int file_size_cid = read_binary(FILE_CIDADAOS, db->cidadaos, MAX_CIDADAOS * sizeof(Cidadao));
    db->num_cidadaos = file_size_cid / sizeof(Cidadao);

    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
    int file_size_enf = read_binary(FILE_ENFERMEIROS, db->enfermeiros, MAX_ENFERMEIROS * sizeof(Enfermeiro));
    db->num_enfermeiros = file_size_enf / sizeof(Enfermeiro);
...
void trata_mensagem_cidadao() {
    ...
    } else if (mensagem.dados.pedido == CANCELADA) {
...
void processa_pedido() {
    ...
    for (; idx_cid < db->num_cidadaos; idx_cid++) {
        if (db->cidadaos[idx_cid].num_utente == mensagem.dados.num_utente && (strcmp(mensagem.dados.nome, db->cidadaos[idx_cid].nome)) == 0) {
            resposta.dados.cidadao = db->cidadaos[idx_cid];
            indicador = 1;
            if (db->cidadaos[idx_cid].estado_vacinacao >= 2) {
                resposta.dados.status = VACINADO;
            }
            if (db->cidadaos[idx_cid].PID_cidadao > 0) {
                resposta.dados.status = EMCURSO;
            } else {
                db->cidadaos[idx_cid].PID_cidadao = mensagem.dados.PID_cidadao;
            }
            sucesso("S5.1) Cidadão %d, %s encontrado, estado_vacinacao=%d, status=%d", db->cidadaos[idx_cid].num_utente, db->cidadaos[idx_cid].nome, db->cidadaos[idx_cid].estado_vacinacao, resposta.dados.status);
            break;
        }
    }
    ...
    char CS[100];
    sprintf(CS, "CS%s", resposta.dados.cidadao.localidade);
    if (resposta.dados.status != DESCONHECIDO && resposta.dados.status != VACINADO && resposta.dados.status != EMCURSO) {
        for (int idx_enf = 0; idx_enf < MAX_ENFERMEIROS; idx_enf++) {
            if (strcmp(CS, db->enfermeiros[idx_enf].CS_enfermeiro) == 0) {
                indicador = 0;
                ...
                if (db->enfermeiros[idx_enf].disponibilidade == 0) {
                    resposta.dados.status = AGUARDAR;
                    break;
                } else {
                    reserva_vaga(idx_cid, idx_enf);
                    if (vaga_ativa == -1) {
                        resposta.dados.status = AGUARDAR;
    ...
    if (resposta.dados.status == OK) {
        ...
        db->cidadaos[idx_cid].PID_cidadao = -1;
...
void vacina() {
    ...
    pid_t id = fork();
    exit_on_error(id, "S6.1) Não foi possível criar um novo processo");
    ...
    if (id == 0) { // Processo FILHO
        ...
        sucesso("S6.1) Criado um processo filho com PID_filho=%d", id);
        db->vagas[vaga_ativa].PID_filho = id;
        resposta.dados.status =           0;
    }
}
...
void servidor_dedicado() {
    ...
    int idx_enf = db->vagas[vaga_ativa].index_enfermeiro;
    db->enfermeiros[idx_enf].disponibilidade = 0;
    ...
    int idx_cid = db->vagas[vaga_ativa].index_cidadao;
    sucesso("S7.4) Vacina em curso para o cidadão %d, %s, e com o enfermeiro %d, %s na vaga %d", db->cidadaos[idx_cid].num_utente, db->cidadaos[idx_cid].nome, db->enfermeiros[idx_enf].ced_profissional, db->enfermeiros[idx_enf].nome, vaga_ativa);
    ...
    db->cidadaos[idx_cid].estado_vacinacao++;
    ...
    db->enfermeiros[idx_enf].nr_vacinas_dadas++;
    // S7.6) Atualiza os dados do enfermeiro (coloca disponibilidade=1) na BD de Enfermeiros;
    db->enfermeiros[idx_enf].disponibilidade = 1;
    // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
    sucesso("S7.6) Cidadão atualizado na BD para estado_vacinacao=%d, Enfermeiro atualizado na BD para nr_vacinas_dadas=%d e disponibilidade=%d", db->cidadaos[idx_cid].estado_vacinacao, db->enfermeiros[idx_enf].nr_vacinas_dadas, db->enfermeiros[idx_enf].disponibilidade);
...
int reserva_vaga(int index_cidadao, int index_enfermeiro) {
    ...
    for (int idx_vaga = 0; idx_vaga < MAX_VAGAS; idx_vaga++) { // Itera-se por todas as vagas de modo a encontrar a primeira vaga não preenchida
        if (db->vagas[idx_vaga].index_cidadao < 0) {
...
void cancela_pedido() {
    ...
    for (int idx_vaga = 0; idx_vaga < MAX_VAGAS; idx_vaga++) {
        int idx_cid = db->vagas[idx_vaga].index_cidadao;
        if (db->cidadaos[idx_cid].num_utente == mensagem.dados.num_utente && strcmp(db->cidadaos[idx_cid].nome, mensagem.dados.nome) == 0) {
            sucesso("S10.1) Foi encontrada a sessão do cidadão %d, %s na sala com o index %d", db->cidadaos[db->vagas[idx_vaga].index_cidadao].num_utente, db->cidadaos[db->vagas[idx_vaga].index_cidadao].nome, vaga_ativa);
            ...
            // Outputs esperados (itens entre <> substituídos pelos valores correspondentes):
            sucesso("S10.2) Enviado sinal SIGTERM ao Servidor Dedicado com PID=%d", db->vagas[vaga_ativa].PID_filho);

            return;
        }
    }
    erro("S10.1) Não foi encontrada nenhuma sessão do cidadão %d, %s", db->cidadaos[db->vagas[vaga_ativa].index_cidadao].num_utente, db->cidadaos[db->vagas[vaga_ativa].index_cidadao].nome);
}
...
void termina_servidor(int sinal) {
    ...
    for (int idx_vaga = 0; idx_vaga < MAX_VAGAS; idx_vaga++) { // Percorre-se todas as vagas
        if (db->vagas[idx_vaga].PID_filho > 0) { // Caso o PID_filho da vaga[i] for maior que 0
            db->enfermeiros[db->vagas[idx_vaga].index_enfermeiro].disponibilidade = 1;
            db->cidadaos[db->vagas[idx_vaga].index_cidadao].PID_cidadao = -1;
            kill(db->vagas[idx_vaga].PID_filho, SIGTERM); // Envia um sinal do tipo SIGTERM ao filho
        }
    }
